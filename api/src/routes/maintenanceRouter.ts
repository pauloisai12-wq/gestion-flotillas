...
const storage = multer.diskStorage({
  destination: function(_req, _file, cb) {
    cb(null, UPLOAD_DIRS.maintenance);
  },
...
router.get('/pending', roleMiddleware(RoleGroups.MAINTENANCE_READERS), ah(async function(_req: Request, res: Response) {
  const pending = await getAllPendingServices();
  res.json({ data: pending });
}));

// GET /api/maintenance/upcoming/:vehicleId — Próximos servicios de un vehículo
router.get('/upcoming/:vehicleId', roleMiddleware(RoleGroups.MAINTENANCE_READERS), ah(async function(req: Request, res: Response) {
...
router.get('/', roleMiddleware(RoleGroups.MAINTENANCE_READERS), validateQuery(listMaintenanceQuerySchema), ah(async function(req: Request, res: Response) {
...
router.get('/vehicle/:vehicleId', roleMiddleware(RoleGroups.MAINTENANCE_READERS), validateQuery(maintenanceHistoryQuerySchema), ah(async function(req: Request, res: Response) {
  const vehicleId = parseId(req, 'vehicleId');
  const result = await maintenanceRecordService.getByVehicle(
    vehicleId,
    req.query as unknown as MaintenanceHistoryQuery,
  );
  res.json(result);
}));
...
router.post('/', roleMiddleware(['ADMIN', 'SUPERVISOR_VEHICLES']), uploadRateLimit, upload.single('evidence'), ah(async function(req: Request, res: Response) {
...
router.put('/:id', roleMiddleware(['ADMIN', 'SUPERVISOR_VEHICLES']), uploadRateLimit, upload.single('evidence'), ah(async function(req: Request, res: Response) {
