...
import {
  ...
  getReportHistory,
  getReportById,
  serializeReportHistoryForClient,
...
const downloadReport = ah(async (req: Request, res: Response) => {
  ...
  if (!sent) res.status(404).json({ error: 'Archivo no encontrado en disco' });
});
...
reportRouter.get(
  ...
  ah(async (req: Request, res: Response) => {
    ...
    res.json({ data: serializeReportHistoryForClient(report) });
  })
);

// HEAD/GET /api/reports/:id/download/:type — Descargar PDF o Excel (ADMIN)
reportRouter.head(
  '/:id/download/:type',
  roleMiddleware(['ADMIN']),
  downloadReport,
