// Revoca (desactiva) uno o más dispositivos de Encuestas Okrean por id o por
// nombre.
//
//   $COMPOSE run --rm -e DEVICE_ID=3 api node dist/scripts/encuestas-device-revoke.js
//   $COMPOSE run --rm -e DEVICE_NAME="encuestador-01" api node dist/scripts/encuestas-device-revoke.js
...
async function main(): Promise<void> {
  ...
  if (idRaw) {
    ...
    ({ count } = await prisma.encuestaDispositivo.updateMany({
      where: { id },
      data: { activo: false },
    }));
  } else {
